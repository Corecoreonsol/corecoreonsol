...
const CONFIG = {
  ...
  // API Configuration
  api: {
    dexscreener: `https://api.dexscreener.com/latest/dex/tokens/4FdojUmXeaFMBG6yUaoufAC5Bz7u9AwnSAMizkx5pump`,
    jupiter: `https://price.jup.ag/v4/price?ids=4FdojUmXeaFMBG6yUaoufAC5Bz7u9AwnSAMizkx5pump`,
    solscan: `https://public-api.solscan.io/token/meta?tokenAddress=4FdojUmXeaFMBG6yUaoufAC5Bz7u9AwnSAMizkx5pump`,
...
function init() {
  ...
  // Note: Typewriter effect is now handled by scroll-animations.js
  // initializeTypewriter(); // Commented out to prevent conflicts
...
function handleNavClick(e) {
  ...
  // If no data-section, let the link work normally (for external links like BUY NOW)
}
...
function copyContractAddress() {
  ...
  const footerContractElement = document.getElementById('footer-contract');
  
  if (navigator.clipboard) {
    navigator.clipboard.writeText(address).then(() => {
      // Change button content to COPIED with checkmark (centered)
      if (contractTextElement) {
        const buttonElement = contractTextElement.closest('button');
        
        if (buttonElement) {
          const originalHTML = buttonElement.innerHTML;
          buttonElement.innerHTML = 'COPIED ✓';
          
          // Reset back to original content after 2 seconds
          setTimeout(() => {
            buttonElement.innerHTML = originalHTML;
          }, 2000);
        }
      }
      if (footerContractElement) {
        // Footer contract address - just copy, don't change text
        // The copying is already handled above
      }
      playSound('success');
    }).catch(() => {
...
function fallbackCopyTextToClipboard(text) {
  ...
  try {
    ...
    if (successful) {
      ...
      // Keep original error handling but without notification
...
function initializeAnimations() {
  // Counter animation is now handled by scroll-animations.js
  // animateCounter(); // Commented out to prevent conflicts
...
/**
 * Refresh live statistics data from multiple APIs
 */
async function refreshLiveData() {
  console.log('🔄 Refreshing live data...');
  
  // Prevent multiple concurrent requests
  if (state.liveData.isLoading) {
  ...
  try {
    ...
    state.liveData.error = error.message;
    
    // Show fallback data or error state
    updateErrorState();
    console.warn('⚠️ Using cached or placeholder data due to API error');
  }
}

/**
 * Fetch data from multiple API sources with fallbacks
 */
  ...
  for (const fetchFunction of sources) {
    try {
      ...
      if (data && data.price > 0) {
        console.log(`✅ Data fetched successfully from ${fetchFunction.name}`);
        return data;
      }
    } catch (error) {
      console.warn(`⚠️ Failed to fetch from ${fetchFunction.name}:`, error.message);
      continue;
    }
  }
...
async function fetchFromDexScreener() {
  const response = await fetch(CONFIG.api.dexscreener, {
    method: 'GET',
    headers: {
...
async function fetchFromJupiter() {
  const response = await fetch(CONFIG.api.jupiter, {
    method: 'GET',
    headers: {
      'Accept': 'application/json',
    }
  ...
  if (data.data && data.data[CONFIG.contractAddress]) {
    const tokenData = data.data[CONFIG.contractAddress];
    
    return {
      price: parseFloat(tokenData.price) || 0,
      marketCap: parseFloat(tokenData.price) * CONFIG.totalSupply || 0,
      volume24h: 0, // Jupiter doesn't provide volume
      priceChange24h: 0, // Jupiter doesn't provide price change
      source: 'Jupiter'
    };
  }
...
/**
 * Fetch data from Birdeye API (alternative source)
 */
async function fetchFromBirdeye() {
  const birdeyeUrl = `https://public-api.birdeye.so/public/price?address=${CONFIG.contractAddress}`;
  ...
  if (data.data && data.data.value) {
    return {
      ...
      marketCap: parseFloat(data.data.value) * CONFIG.totalSupply || 0,
      volume24h: 0, // Birdeye base endpoint doesn't provide volume
      priceChange24h: 0, // Would need additional API call
      source: 'Birdeye'
    };
  }
...
function updateLoadingState(isLoading) {
  const elements = {
    ...
    volume: document.getElementById('volume-24h'),
    priceChange: document.getElementById('price-change'),
    mcapChange: document.getElementById('mcap-change'),
...
function updateLiveDataUI(data) {
  ...
  if (mcapElement) {
    mcapElement.textContent = `$${formatLarge(data.marketCap)}`;
  }
  
  // Update volume
  const volumeElement = document.getElementById('volume-24h');
  if (volumeElement) {
    volumeElement.textContent = `$${formatLarge(data.volume24h)}`;
  }
  
  // Update price change
  const priceChangeElement = document.getElementById('price-change');
  if (priceChangeElement) {
    priceChangeElement.textContent = formatChange(data.priceChange24h);
    priceChangeElement.style.color = data.priceChange24h >= 0 ? '#00D4AA' : '#ff4444';
  }
  
  // Update other change indicators (if they exist)
  const mcapChangeElement = document.getElementById('mcap-change');
  if (mcapChangeElement) {
    mcapChangeElement.textContent = formatChange(data.priceChange24h); // Same as price change
    mcapChangeElement.style.color = data.priceChange24h >= 0 ? '#00D4AA' : '#ff4444';
  }
  
  const volumeChangeElement = document.getElementById('volume-change');
  if (volumeChangeElement) {
    volumeChangeElement.textContent = '+0.00% 24H'; // Placeholder
    volumeChangeElement.style.color = '#888888';
  }
...
function updateErrorState() {
  ...
  // Show placeholder or last known values
  if (state.liveData.price > 0) {
    updateLiveDataUI(state.liveData);
  } else {
...
function startAutoRefresh() {
  ...
  console.log(`🔄 Auto-refresh started (${CONFIG.api.refreshInterval / 1000}s interval)`);
}
